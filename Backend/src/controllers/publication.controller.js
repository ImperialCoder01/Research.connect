...
export const createPublication = async (req, res, next) => {
  try {
    ...
    const publication = await Publication.create({
      title, abstract, authors, journal, doi, publicationDate, fileUrl, tags, citationCount,
    });
...
export const getAllPublications = async (req, res, next) => {
  try {
    ...
    if (author) filter['authors.displayName'] = { $regex: author, $options: 'i' };

    const allowedSort = ['publicationDate', 'citationCount', 'title', 'createdAt'];
    const sortField = allowedSort.includes(sortBy) ? sortBy : 'createdAt';
    const sortOrder = order === 'asc' ? 1 : -1;

    const [publications, total] = await Promise.all([
      Publication.find(filter).sort({ [sortField]: sortOrder }).skip(skip).limit(l).lean(),
...
// GET /api/v1/publications/search?q=
export const searchPublications = async (req, res, next) => {
  try {
    const { q, page, limit } = req.query;
    if (!q || !q.trim()) return next(new AppError('Search query (q) is required', 400));
    const { page: p, limit: l, skip } = paginate(page, limit);
    const filter = {
      $or: [
...
export const getPublicationById = async (req, res, next) => {
  try {
    ...
    if (!publication) return next(new AppError('No publication found with that ID', 404));
    res.status(200).json({ status: 'success', data: { publication } });
  } catch (err) {
    next(err);
  }
};
